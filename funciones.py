...
def calcularValorMano (listajugadores):
    ...
    return valormano
# Procedimiento que llama a la función calcularValorMano y añade el valor que se da (valor mano jugador) a la lista del jugador.
...
def verBanca (listabanca):
    ...
    for i in listabanca[1]:
        if i != listabanca[1][-1]:
            ...
            print("%s" % (i), end="")
    prints.colorreset()
# Procedimiento que muestra las cartas de todos los jugadores en la mesa incluida la banca. 
# Se va utilizando según los jugadores obtengan cartas nuevas, así como resaltando el jugador actual y ocultando la última carta del resto de jugadores. 
        ...
        if i[0] == jugador:
            ...
            for j in i[3]:                          # Aquí imprimirá las cartas del resto, separado ya que no debemos ver la última
                if j != i[3][-1]:                   # carta del resto.
                    ...
                    print("?", end="")
        prints.colorreset()
# Procedimiento que muestra todas las cartas de los jugadores más la banca. Se utiliza al final de la ronda, mostrando la mano final de la banca y todas las cartas de los jugadores.
def verMesa(listajugadores,listabanca):
    prints.mesa()
    verBanca(listabanca)
    for i in listajugadores:
        if listabanca[2] > 21 and i[4] <= 21:       # Determinar si el jugador ha ganado o perdido en función de sí su puntuación no supera
            estado = "GANADOR/A"                    # 21 y la de la banca si.
            prints.colorganador()
        elif i[4] > listabanca[2] and i[4] <= 21:   # Determinar si el jugador ha ganado o perdido en función de sí su puntuación supera a la
        ...
        for j in i[3]:
            if j != i[3][-1]:                   # Se enseñan todas las cartas, ya que es el final de la partida.
                ...
                print("%s" % (j), end="")
        prints.colorreset()
    if listabanca[0]>0: # Comprueba si la banca se ha quedado sin dinero, para lanzar el mensaje de bancarrota o fin de ronda.
        print("")
        prints.colorinput("La ronda ha finalizado, pulsa \"ENTER\" para continuar.")
    else:
        print("\n   >>> \033[31mBANCARROTA! \033[92mLa banca se ha quedado sin dinero, coge el dinero antes de que llegue seguridad.")
...
def eliminarDatosRonda (listajugadores):
    for i in listajugadores:
        del i[2:]
# Función utilizada para acortar código en la función de opcionesJugadores(...).
...
# Función utilizada en el menú de opciones para refrescar las variables mostradas en el menú después de actualizarlas.
...
def modificarBarajas(baraja):
    ...
    while baraja_final < 1:         # Verificar que haya al menos 1 baraja para jugar.
